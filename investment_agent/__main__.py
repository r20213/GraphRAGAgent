"""Local runtime entry point for the Investment Research Agent.

Two modes are supported:

* ``python -m investment_agent`` (default) — interactive REPL backed by an
  in-memory ADK runner; handy for smoke-testing tool wiring without A2A.
* ``python -m investment_agent --a2a`` — serves the A2A ASGI app with uvicorn,
...
async def _run_repl() -> None:
    ...
    print("Investment Research Agent — local test loop.")
...
def _serve_a2a() -> None:
    ...
    uvicorn.run("investment_agent.a2a:a2a_app", host=host, port=port)
