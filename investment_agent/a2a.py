"""A2A (Agent-to-Agent) protocol surface for the Investment Research Agent.

Wraps :data:`investment_agent.agent.root_agent` in an A2A-compliant ASGI
...
* ``/.well-known/agent-card.json`` — the public Agent Card describing the
  agent's identity, skills, and JSON-RPC endpoint, used by A2A clients for
  capability discovery.
* the A2A JSON-RPC endpoint that other agents call to create and stream tasks.

Run locally::

    uvicorn investment_agent.a2a:a2a_app --host 0.0.0.0 --port 8080
